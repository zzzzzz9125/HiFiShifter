...
#ifdef _WIN64
	#pragma comment( lib, "vslib_x64.lib" )
#else
...
// 解析オプション
#define ANALYZE_OPTION_SKIP_PIT_ANALYZE		(0x00000001)	// PIT解析をスキップ
#define ANALYZE_OPTION_WORLD				(0x00000000)	// WORLD
#define ANALYZE_OPTION_VOCAL_SHIFTER		(0x00000002)	// VocalShifter
...
// トラック情報(EX)
typedef struct{
	char name[64];			// (R/W) トラック名
	double volume;			// (R/W) ボリューム[倍]
	double pan;				// (R/W) パン(-1.0～1.0)
	double reserved1[2];	// 予約
	int invPhaseFlg;		// (R/W) 逆相フラグ
	int soloFlg;			// (R/W) ソロフラグ
	int muteFlg;			// (R/W) ミュートフラグ
	int color;				// (R/W) トラックカラー
	int morphGroup;			// (R/W) モーフィンググループ
	unsigned int option;	// (R/W) トラックオプション
	int reserved2[10];		// 予約
} VSTRACKINFOEX;
...
VSLIB_IMPORT int __stdcall VslibGetTrackInfoEx( HVSPRJ hVsprj, int trackNum, VSTRACKINFOEX *vsTrackInfo );
VSLIB_IMPORT int __stdcall VslibSetTrackInfoEx( HVSPRJ hVsprj, int trackNum, VSTRACKINFOEX *vsTrackInfo );
...
VSLIB_IMPORT int __stdcall VslibAddItemEx( HVSPRJ hVsprj, const char *fileName, int *itemNum,
									int nnOffset, int nnRange, unsigned int option );
...
VSLIB_IMPORT int __stdcall VslibGetCtrlPntInfoEx2( HVSPRJ hVsprj, int itemNum, int pntNum, VSCPINFOEX2 *vsCpInfo2 );
VSLIB_IMPORT int __stdcall VslibSetCtrlPntInfoEx2( HVSPRJ hVsprj, int itemNum, int pntNum, VSCPINFOEX2 *vsCpInfo2 );
...
VSLIB_IMPORT int __stdcall VslibGetHEQGain( HVSPRJ hVsprj, int itemNum, int *gain );
VSLIB_IMPORT int __stdcall VslibSetHEQGain( HVSPRJ hVsprj, int itemNum, int *gain );
